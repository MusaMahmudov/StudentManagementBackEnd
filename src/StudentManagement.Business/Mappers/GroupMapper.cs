...
namespace StudentManagement.Business.Mappers
{
    public class GroupMapper : Profile
    {
        public GroupMapper() 
        {
            CreateMap<Group,GetGroupForObjectsUpdateDTO>().ReverseMap();
            ...
            CreateMap<Group,GetMainGroupForStudentPageDTO>().ForMember(gg=>gg.facultyName,x=>x.MapFrom(g=>g.Faculty.Name)).ForMember(gg=>gg.Students,x=>x.MapFrom(g=>g.Students)).ReverseMap();
            CreateMap<Group, GetMainGroupForStudentDTO>().ForMember(gg=>gg.facultyName,x=>x.MapFrom(g=>g.Faculty.Name)).ReverseMap();
            CreateMap<Group,GetGroupForUpdateDTO>().ForMember(gg=>gg.StudentsId,g=>g.MapFrom(g=>g.Students.Select(s=>s.Id))).ReverseMap();
            ...
            CreateMap<PostGroupDTO, Group>().ForMember(g => g.FacultyId, x => x.MapFrom(pg => pg.FacultyId)).ReverseMap();
            CreateMap<PutGroupDTO, Group>().ForMember(g => g.FacultyId, x => x.MapFrom(pg => pg.FacultyId)).ReverseMap();
